    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          <h4 className="font-bold text-slate-800 text-sm mb-4">Asset Status Utilization</h4>
          <div className="h-64 flex flex-col justify-center items-center">
            {utilizationData.length > 0 ? (
              <ResponsiveContainer width="100%" height="80%">
                <PieChart>
                  ...
                  <RechartsTooltip contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", color: "#0f172a" }} />
                </PieChart>
              </ResponsiveContainer>
            ) : (
            ...
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-2">
              {utilizationData.map((entry, i) => (
                <div key={entry.name} className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase">
        ...
        <Card className="lg:col-span-2">
          <h4 className="font-bold text-slate-800 text-sm mb-4">Department Asset Allocation</h4>
          <div className="h-64">
            {deptData.length > 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <h4 className="font-bold text-slate-800 text-sm mb-4">Maintenance Spend by Priority</h4>
          <div className="h-64">
            {maintData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={maintData}>
                  ...
                  <RechartsTooltip contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", color: "#0f172a" }} />
                  <Legend verticalAlign="top" height={36} iconType="circle" />
                  <Area type="monotone" dataKey="cost" name="Service Cost ($)" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.05} />
                  <Area type="monotone" dataKey="count" name="Tickets count" stroke="#ef4444" fill="#ef4444" fillOpacity={0.05} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
        ...
        <Card className="lg:col-span-1">
          <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
            <h4 className="font-bold text-slate-800 text-sm">Upcoming Returns</h4>
            <Link to="/allocations" className="text-xs text-indigo-650 hover:underline flex items-center gap-1 font-semibold">
              ...
              upcomingReturns.map((alloc) => (
                <div key={alloc.id} className="py-3 flex justify-between items-center text-xs">
                  ...
                  <div className="text-right">
                    <span className="inline-flex items-center gap-1 font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
