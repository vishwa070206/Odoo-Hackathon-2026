  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Departments Setup</h1>
          <p className="text-sm text-slate-400">Configure your organization chart, assign department managers, and view structural units.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {departments.length > 0 ? (
              ...
              <Card key={dept.id} className="flex flex-col justify-between space-y-4">
                <div>
                  <div className="flex justify-between items-start gap-2">
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-450 border border-emerald-500/20 text-[10px] font-bold font-mono">
                      {dept.code}
                    </span>
                  ...
                  <h3 className="text-sm font-bold text-slate-100 mt-2">{dept.name}</h3>
                  <p className="text-xs text-slate-450 line-clamp-2 mt-1">{dept.description || "No description provided."}</p>

                  <div className="flex flex-col gap-1.5 pt-4 mt-4 border-t border-slate-850 text-xs text-slate-400">
            ...
            <Card className="col-span-3 text-center py-20">
              ...
              <h3 className="text-lg font-bold text-slate-350">No Departments Found</h3>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-bold text-slate-100">Add Department Unit</h3>
            ...
            <form onSubmit={handleCreateDepartment} className="space-y-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Parent Department (Optional)</label>
                <select
                  value={parentId}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Department Head / Manager</label>
                <select
                  value={headId}
